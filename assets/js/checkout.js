...
(() => {
  ...
  const initAddressPage = () => {
    ...
    calculateButton?.addEventListener('click', async () => {
      ...
      const cep = normalizeDigits(fields.addressCep?.value || '');
      if (cep.length !== 8) {
      ...
      const originalLabel = calculateButton.innerHTML;
      ...
      try {
        ...
        const freight = data.cotacao;
        if (!freight?.options?.length) {
          throw new Error('Nenhuma opção de entrega retornada.');
        }
        ...
        showStatus('Opções de entrega atualizadas pela API.', 'success');
        toast({
        ...
        calculateButton.innerHTML = originalLabel;
      }
    });
  ...
  const finalizeOrder = async (paymentMethod, paymentDetails = {}) => {
    ...
    try {
      ...
      const apiOrder = data.pedido || {};
      const order = {
        orderNumber: apiOrder.numeroPedido || generateOrderNumber(),
        ...
        shipping: Number(apiOrder.frete ?? shipping ?? 0),
        total: Number(apiOrder.total ?? total),
        status: apiOrder.status || 'aguardando_pagamento',
      };
      ...
      toast({
        ...
        message: 'A compra foi registrada pela API.',
        icon: 'fa-solid fa-circle-check',
      });
