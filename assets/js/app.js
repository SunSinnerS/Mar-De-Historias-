...
(() => {
  ...
  const syncSessionFromApi = async () => {
    try {
      ...
      if (data?.usuario) {
        writeSessionSnapshot(data.usuario);
      }
    } catch (error) {
      // Mantém a interface funcional mesmo se a API estiver indisponível.
    }
  ...
  document.addEventListener('lmh:catalog-rendered', () => {
    initHomeCartButtons();
    initFavoriteButtons();
  });
